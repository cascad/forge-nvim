...
return {
    {
        ...
        opts = {
            formatters_by_ft = {
                ...
                -- Go: ТОЛЬКО gofumpt, БЕЗ goimports. goimports автоматически
                -- добавляет недостающие импорты эвристикой выбора пакета —
                -- которая регулярно ошибается на коллизиях (rand, log, parse
                -- и т.п.). Импорты добавляются явно через <leader>la
                -- (code action) или <leader>lO (organize imports — оба
                -- идут через gopls, который при неоднозначности предлагает
                -- ВЫБОР, а не угадывает).
                --
                -- Если для конкретного проекта нужен старый goimports-флоу
                -- (например, монорепы где коллизий нет), можно вернуть в
                -- ~/.config/nvim/lua/plugins/format.lua override или
                -- сделать :let g:disable_autoformat=1 и звать руками.
                go     = { "gofumpt" },

...
-- =============================================================
-- DAP UI (nvim-dap-ui) — обёртки для panels.lua-функций, которые
-- раньше дёргали `:DapViewOpen` / `:DapViewShow`. Теперь мы поверх
-- ide framework + dap-ui:
--   section == "console"                     → ide.show("dap.console") (layout 2 — bottom)
--   section == "repl"                        → ide.show("dap.repl")    (layout 3 — bottom)
--   section in { scopes/watches/stacks/threads/breakpoints } → ide.show("dap.sidebar") (layout 1 — left)
--                                              + фокус на нужное окно по filetype dapui_<section>
...
function M.close_debug()
    ...
    if ide then
        ...
        -- Fallback (ide ещё не загрузился).
...
-- @param section nil | "console" | "repl" | "scopes" | "watches" | "stacks" | "threads" | "breakpoints"
--   nil          — открыть ТОЛЬКО left sidebar; bottom не трогаем (нужно при
--                  F5 до получения адаптерного типа, чтобы не мелькало
--                  console→repl).
--   "console"    — sidebar + bottom console (Rust, codelldb).
--   "repl"       — sidebar + bottom repl (Go, Python+internalConsole).
--   sidebar-ft   — sidebar; bottom не трогаем; фокус на конкретное окно.
function M.show_debug_section(section, enter)
    ...
    elseif section == "terminal" then
        ...
        -- Если пытаться открыть сейчас, edgy подхватит пустой буфер
        -- БЕЗ termopen, и потом будет конфликт при runInTerminal-вызове.
        -- Закрываем предыдущий bottom (на случай, если там dap.repl от
        -- прошлой Go/Python сессии).
        require("ide").hide_slot("bottom")
    end
    -- nil или sidebar_section_ft: bottom не трогаем — он откроется
    -- позже (через event_initialized listener в plugins/dap.lua),
    -- когда тип адаптера известен.
...
local function focus_entry(entry)
    if entry.show_debug then
        -- В dap-ui нет одного окна с табами — каждая секция —
        -- отдельный буфер с filetype dapui_<id>. show_debug_section
        -- открывает нужный layout и (для sidebar-секций) ставит фокус
        -- на конкретное окно.
        M.show_debug_section(entry.show_debug, true)
        return true
    end
...
-- Циклически перейти к next/prev DAP-окну (любой dapui_* / dap-repl)
-- в текущем tabpage. В dap-ui секции — это отдельные окна, поэтому
-- "Navigate" = wincmd-переключение между ними.
local function cycle_debug_windows(dir)
    local wins = {}
    for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
        if vim.api.nvim_win_is_valid(win) then
            local buf = vim.api.nvim_win_get_buf(win)
            if is_debug_buffer(buf) then table.insert(wins, win) end
        end
    end
    ...
    local idx = 0
    for i, w in ipairs(wins) do if w == current then idx = i; break end end
    local next_idx = ((idx - 1 + dir) % #wins) + 1
    if idx == 0 then next_idx = (dir > 0) and 1 or #wins end
    pcall(vim.api.nvim_set_current_win, wins[next_idx])
    return true
end
...
function M.next_mode_window()
    if M.current_mode == "debug" or M.current_mode == "debug-output" then
        ...
        if is_debug_buffer(buf) then
            if cycle_debug_windows(1) then return end
        end
    end

...
**Ошибки/варнинги под курсором** (сами не раскрываются — только знак слева и подчёркивание):

| Клавиши | Действие |
|---|---|
| `gl` (рус. `пд`) | текст ошибки **оверлеем** (окно поверх кода) + подсветка ТОГО САМОГО места, куда ругается компилятор |
| `gl` ещё раз | провалиться В окошко (скроллить длинный текст, копировать) |
| `Esc` | убрать окошко, **не двигая курсор** (второй `Esc` — снять подсветку поиска) |
| `q` | то же, если курсор уже внутри окошка |
| `Ctrl+w d` | то же окно, но без подсветки места (родной дефолт Neovim) |
| `<leader>td` | вернуть inline-режим: off → текущая строка → все строки → off |
| `F3` / `Shift+F3` | следующая / предыдущая диагностика |
